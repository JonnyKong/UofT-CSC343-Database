...
FROM cabinet c1 JOIN cabinet c2 ON c1.id=c2. previous_cabinet_id AND c1.id > c2.id;
...
SELECT c.id AS cabinet_id, c.country_id, c.start_date, e.endDate FROM cabinet c LEFT JOIN end_time e ON c.id = e.cabinet_id;
...
SELECT w.countryName, w.cabinetId, w.startDate, w.endDate,  p.name AS pmParty
